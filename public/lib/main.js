...
(function () {
  gsap.core.globals("ScrollTrigger", ScrollTrigger); // const imgElem=document.querySelector('.intro');
  // const body=document.querySelector('body');

  var cards = document.querySelectorAll(".main_cards .card");
  var myProject = document.querySelector(".myproject");
  var bar = document.querySelector(".loading_bar");
  ...
  advantage.to('.advantage .main_title', 0.2, {
    opacity: 1,
    x: "0px"
  }).to('.txt1', 0.1, {
    y: 200,
  ...
  advantage.to('.ad1', 0.4, {
    scale: 1.1
  });
  advantage.to('.text.txt-2', 0.3, {
    opacity: 0
  });
  advantage.to('.advantages', 0.4, {
    left: "-100%"
  });
  advantage.to('.text.txt-3', 0.3, {
    opacity: 1
  });
  advantage.to('.ad2', 0.4, {
    scale: 1.1
  });
  advantage.to('.text.txt-3', 0.3, {
    opacity: 0
  ...
  advantage.to('.ad3', 0.4, {
    scale: 1.1
  });
  advantage.to('.text.txt-4', 0.3, {
    opacity: 1
  });
  advantage.to('.text.txt-4', 0.3, {
    opacity: 0
  ...
  project.to('.myproject .main_title', 0.2, {
    opacity: 1,
    x: "0px"
  });
  project.to('.myproject .layer_bg', 0.3, {
    opacity: 0.2
  });
  project.to('.myproject .bulb1', 0.1, {
    y: -140
  });
  project.to('.myproject .bulb2', 0.1, {
    y: -100
  });
  project.to('.myproject .bulb3', 0.1, {
    y: -70
  });
  project.to('.myproject .bulb1 .on', 0.1, {
    opacity: 1
  });
  project.to('.myproject .bulb2 .on', 0.1, {
    opacity: 1
  });
  project.to('.myproject .bulb3 .on', 0.1, {
    opacity: 1
  });
  project.to('.card.library', 0.3, {
    opacity: 1,
    x: "0%",
    y: "0%",
    visibility: "visible",
  ...
  var _loop = function _loop(i) {
    ...
    cards[i].addEventListener("mouseenter", function (e) {
      e.preventDefault();
      console.log("click");
      myProject.classList.add("active".concat(i));

      if (prevIndex !== i) {
        myProject.classList.remove("active".concat(prevIndex));
      }
  ...
  var scrollValue = function scrollValue() {
    ...
    var realHeight = scrollHeight - window.innerHeight;
    var realRatio = window.pageYOffset / realHeight * 100;
    if (realRatio < 0) realRatio = 1;
    if (realRatio > 100) realRatio = 100;
    bar.style.width = "".concat(realRatio, "%");
  };
